...
cp = angular.module('mentor', ['ngWYSIWYG', 'ngRoute']);

cp.controller('MentorController', ['$scope', 'allMenuService', '$http', '$log', 'mentorLeftSectionService', '$routeParams',
    function ($scope, allMenuService, $http, $log, mentorLeftSectionService, $routeParams) {
        // side menu
        $scope.getSideMenu = function () {
            //
            var promise = mentorLeftSectionService.getMenuList();
        ...
        $scope.getMenuContent = function (sub_id, index) {
            ...
            $http.get('/static/api/mentorContent.json?menu_id=' + sub_id)
        ...
        $scope.menteePlanner = function(selectedMentee) {
            $scope.showCreateButton = selectedMentee == '' ? false : true;
            $scope.getMenteeHistory(selectedMentee);
        }

        //
        $scope.getMenteeHistory = function (mentee_id) {

            // api call to fetch content of selected menu
            $http.get('/static/api/menteeHistory.json?mentee_id=' + mentee_id)
                    .then(function (response) {
                        $scope.menteeHistory = response.data[mentee_id];
                        console.log($scope.menteeHistory);
                    });
        };
